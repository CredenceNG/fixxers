...
/**
 * Badge Request Form - Client Component
 * Handles form state and submission for badge requests
 */
...
interface BadgeRequestFormProps {
    badge: Badge;
}

export function BadgeRequestForm({ badge }: BadgeRequestFormProps) {
    ...
    function getDocumentLabel(docType: string): string {
        ...
        return docTypeInfo ? docTypeInfo.name : docType.replace(/_/g, ' ');
    }

    function getDocumentIcon(docType: string): string {
        const docTypeInfo = documentTypes.find(dt => dt.id === docType);
        return docTypeInfo ? docTypeInfo.icon : '📄';
    }
